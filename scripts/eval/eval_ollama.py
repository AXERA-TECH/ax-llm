...
BASE_URL = "http://10.122.86.70:11434/v1"
...
def generate_caption_for_image(img_path: str) -> str:
    ...
    system_prompt = open("eval_system_prompt.md", "r").read()
    ...
    messages = [
        ...
        },
        {
            ...
            "content": [
                ...
                {
                    "type": "image_url",
                    "image_url": {"url": image_data},  # 如果你的服务要求 {\"url\": xxx} 再按你那边的格式来
                },
            ],
        }
    ]
...
for i, img_id in tqdm.tqdm(enumerate(img_ids), total=len(img_ids)):
    ...
    if (i + 1) % 50 == 0:
        print(f"Processed {i+1}/{len(img_ids)} images,current caption: {caption}")

# 写出 COCO 官方评测所需的 results.json
with open(f"ollama_{MODEL_NAME}_coco_val2017_results.json", "w", encoding="utf-8") as f:
    json.dump(results, f)
