...
LinearStateSnapshot *LinearStateStore::find(int token_len) {
    for (auto &snapshot : snapshots_)
        if (snapshot.token_len == token_len) return &snapshot;
    return nullptr;
}
...
void Runtime::configure(int layer_count, AttentionConfig config) {
    ...
    if (!config.layer_types.empty()) {
        const int count = std::min(layer_count, static_cast<int>(config.layer_types.size()));
    ...
    if (config.layer_types.empty() || config.num_kv_shared_layers <= 0) return;

    const int type_count = std::min(layer_count, static_cast<int>(config.layer_types.size()));
    const int first_shared = type_count - config.num_kv_shared_layers;
    if (first_shared <= 0) return;
    for (int layer_idx = first_shared; layer_idx < type_count; ++layer_idx) {
        const std::string &type = config.layer_types[static_cast<size_t>(layer_idx)];
...
bool Runtime::is_linear_layer(int layer_idx) const {
    return layer_idx >= 0 && layer_idx < static_cast<int>(linear_layers_.size()) &&
           linear_layers_[static_cast<size_t>(layer_idx)];
}

bool Runtime::has_linear_attention_layers() const {
    return std::any_of(linear_layers_.begin(), linear_layers_.end(), [](bool value) { return value; });
}
...
int Runtime::first_full_layer_idx() const {
    for (int i = 0; i < static_cast<int>(linear_layers_.size()); ++i)
        if (!linear_layers_[static_cast<size_t>(i)]) return i;
    return -1;
}

int Runtime::shared_kv_source_for_layer(int layer_idx) const {
    if (layer_idx < 0 || layer_idx >= static_cast<int>(shared_kv_source_layers_.size())) return -1;
...
std::vector<int> Runtime::token_suffix(const std::vector<int> &cached,
    ...
    while (static_cast<size_t>(common_prefix) < limit &&
           cached[static_cast<size_t>(common_prefix)] == requested[static_cast<size_t>(common_prefix)])
        ++common_prefix;
    if (static_cast<size_t>(common_prefix) >= requested.size()) return {};
    return std::vector<int>(requested.begin() + common_prefix, requested.end());
}

size_t Runtime::cache_bytes(uint32_t n_size) { return static_cast<size_t>(n_size); }
...
bool Runtime::compatible_cache_dtype(int src_dtype, int dst_dtype) {
    // Older AXCL tensor descriptors report zero, meaning unknown.
