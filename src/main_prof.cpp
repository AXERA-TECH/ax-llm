...
// 1. init engine
    ...
    int max_token = attr.tokens_embed_num;
    ALOGI("max_token: %d", max_token);

    auto random_token = [&](int num) -> std::vector<int>
    {
        std::vector<int> tokens_ids;

        for (int i = 0; i < num; i++)
        {
            tokens_ids.push_back(rand() % max_token);
        }

        return tokens_ids;
    };

    std::vector<int> test_case = {20, 500, 1000, 1500, 2000, 2500, 3000, 3328};

    for (auto count : test_case)
    {
        ALOGI("test case: %d", count);
        int precompute_len = 0;
        std::vector<std::vector<unsigned short>> k_caches, v_caches;
        auto _token_ids = random_token(count);

        lLaMa.getAttr()->prefill_max_token_num = lLaMa.getAttr()->prefill_max_kv_cache_num_grp[lLaMa.getAttr()->prefill_max_kv_cache_num_grp.size() - 1];

        timer t;
        lLaMa.GenerateKVCachePrefill(_token_ids, k_caches, v_caches, precompute_len);
        t.stop();
        ALOGI("precompute_len: %d, time: %f", precompute_len, t.cost());

        {
            auto tokens_diff = random_token(20);
            std::vector<unsigned short> prompt_data;
            prompt_data.resize(tokens_diff.size() * lLaMa.getAttr()->tokens_embed_size);
            ...
            auto last_reply = lLaMa.Run(prompt_data, 100);
